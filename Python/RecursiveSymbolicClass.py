...
class RecursiveSymbolicRegression:
    ...
    def fit(self, X, y, it_inter=8, it_inv=5, it_trans=3, thr=1e-4):
        ...
        bestexpr = [term for term, coef in zip(bestexpr, self.model.coef_)] #if np.any(np.abs(coef) > self.thr)]
    ...
    def generateModel(self, X, y, expr):
        ...
        return self.model.score(Xt, y), self.model #np.min(score)
